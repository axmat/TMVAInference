...
template <typename T> class ROperator_LayerNormalization : public ROperator {
private:
  int64_t fAttrAxis;
  float fAttrEpsilon;
...
public:
  ...
  void Initialize(RModel &model) override {
    ...
    fNormalizedShape =
        std::vector<size_t>(fShapeX.begin() + fAxis, fShapeX.end());
  ...
  std::string Generate(std::string OpName) override {
    ...
    for (size_t j = fAxis; j < fSize; j++) {
      ...
      out << SP << SP << "for (size_t " << jIdx << " = 0; " << jIdx << " < "
          << inputShape;
      out << "[" << j << "]; " << jIdx << "++) {\n";
    }
    ...
    out << SP << SP << "tensor_" << fNMean << "[" << axesIndex << "] = sum / "
        << fType << "(";
    ...
    out << SP << SP << SP << "sum += std::pow(tensor_" << fNX << "["
        << InputIndex << "] - tensor_";
    out << fNMean << "[" << axesIndex << "], 2);\n";
    for (size_t j = fAxis; j < fSize; j++) {
      out << SP << SP << "}\n";
    }
    out << SP << SP << "tensor_" << fNInvStdDev << "[" << axesIndex
        << "] = 1 / std::sqrt(";
    out << "sum / " << fType << "(" << fNormalizedLength << ") + "
        << fAttrEpsilon << ");\n";
    for (size_t i = 0; i < fAxis; i++) {
      out << SP << "}\n";
    }
    ...
    out << SP
        << "// Y = Scale o NormalizedInput + B = Scale o InvStdDev (X - Mean) "
           "+ B\n";
    // loop over all the dims in [0, fAxis)
    for (size_t i = 0; i < fAxis; i++) {
      std::string iIdx = "axis_" + std::to_string(i);
    ...
    out << SP << SP << SP << "tensor_" << fNY << "[" << InputIndex
        << "] = tensor_" << fNScale;
    out << "[" << normalizedIndex << "] * tensor_" << fNInvStdDev << "["
        << axesIndex;
...
ParserFuncSignature ParseLayerNormalisation =
  ...
  if (parser.IsRegisteredTensorType(input_name)) {
    ...
    throw std::runtime_error(
        "TMVA::SOFIE ONNX Parser LayerNormalizaion op has input tensor " +
  ...
  case ETensorType::FLOAT:
    op.reset(new ROperator_LayerNormalization<float>(
        axis, epsilon, stash_type, input_name, name_scale, name_bias,
        output_name, name_mean, name_std));
    break;
  default:
    throw std::runtime_error(
        "TMVA::SOFIE ONNX parser Operator with input type " +
        ConvertTypeToString(input_type) + " not supported.");
    break;
  }
