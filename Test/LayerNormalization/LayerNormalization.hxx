//Code generated automatically by TMVA for Inference of Model file [LayerNormalization.onnx] at [Wed Oct 12 13:22:30 2022] 
...
std::vector<float> infer(float* tensor_X){
   ...
   for (size_t axis_0 = 0; axis_0 < op_0_InputShape[0]; axis_0++){
      for (size_t axis_1 = 0; axis_1 < op_0_InputShape[1]; axis_1++){
         tensor_Y[axis_0 * 4 + axis_1 * 1] = tensor_Scale[axis_1 * 1] * tensor_InvStdDev[axis_0 * 1] * (tensor_X[axis_0 * 4 + axis_1 * 1] - tensor_Mean[axis_0 * 1]) + tensor_B[axis_1 * 1];
      }
   }
