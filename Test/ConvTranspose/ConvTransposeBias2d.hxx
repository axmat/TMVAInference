//Code generated automatically by TMVA for Inference of Model file [ConvTransposeBias2d.onnx] at [Fri Oct  7 10:54:08 2022] 
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "ConvTransposeBias2d.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   f.close();
   {
      ...
      std::vector<size_t> newShape = { 2, 5, 5};
      oldShape.resize(newShape.size(), 1.);
      float * newData_ptr = TMVA::Experimental::SOFIE::UTILITY::Unidirectional_broadcast<float>(tensor_B, oldShape, newShape);
      for (int i = 0; i < 1 ; i++)
         std::copy(newData_ptr, newData_ptr + 50, tensor_BroadcastedB + i * 50);
      delete [] newData_ptr;
   }
}
...
//----  operator ConvTranspose op_0
   ...
   int op_0_incx = 1;
   int op_0_incy = 1;
   BLAS::saxpy_(&op_0_size, &op_0_gamma, tensor_BroadcastedB, &op_0_incx, tensor_Y, &op_0_incy);
