//Code generated automatically by TMVA for Inference of Model file [MeanMultidirectionalBroadcast.onnx] at [Wed Oct 26 12:46:05 2022] 

#ifndef TMVA_SOFIE_MEANMULTIDIRECTIONALBROADCAST
#define TMVA_SOFIE_MEANMULTIDIRECTIONALBROADCAST
...
namespace TMVA_SOFIE_MeanMultidirectionalBroadcast{
struct Session {
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "MeanMultidirectionalBroadcast.dat";
}
...
//------ BasicNary operator
   ...
   for (size_t id = 0; id < 24; id++) {
		tensor_Y[id] = (tensor_BroadcastedA[id] + tensor_BroadcastedB[id] + tensor_BroadcastedC[id]) / float(3);
   }
   std::vector<float> ret (tensor_Y, tensor_Y + 24);
   return ret;
}
};
} //TMVA_SOFIE_MeanMultidirectionalBroadcast

#endif  // TMVA_SOFIE_MEANMULTIDIRECTIONALBROADCAST
