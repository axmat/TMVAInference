 ...
 */
template <typename T>
class ConvTransposeOp final : public ROperator {
private:
   ...
   bool fPad = false;
   std::string fNPaddedX;
   std::vector<size_t> fShapePaddedX;

   bool fDilateW = false;
   std::string fNDilatedW;
   std::vector<size_t> fShapeDilatedW;

   std::string fNF; // Transposed (dilated) kernel matrix
   std::vector<size_t> fShapeF;

   std::string fNVecW;
   std::vector<size_t> fShapeVecW;

public:
   /*! Default constructor of ConvTransposeOp */
   ConvTransposeOp() {}

   /*! \brief Constructor of ConvTransposeOp from the attributes
    *
    ...
    */
   ConvTransposeOp(std::string autopad, std::vector<size_t> dilations, size_t group,
...
// Implementation of the ConvTransposeOp class
#include "ConvTransposeOp.icc"
